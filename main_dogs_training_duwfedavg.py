...
N = sum([3295, 3345, 1868, 3178, 4773])     # number of all training data
if environment=="A":
...
for itr in range(T):
    labelname = 'round ' + str(itr)
    plt.plot([i for i in learnedweights[:,itr,4]], label=labelname)     # trajectory of client 0's weight for each round
plt.legend()
plt.xlabel("iteration m", fontsize=16)
plt.ylabel("theta4", fontsize=16)
plt.tick_params(labelsize=16)
...
torch.save(learnedweights[M,:,:], 'learnedweights_duwfedavg.pt')